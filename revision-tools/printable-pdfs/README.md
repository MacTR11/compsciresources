...
The PDFs are generated from the Markdown sources with [`build-pdfs.py`](../build-pdfs.py) (Markdown → HTML → PDF via headless Chromium). If you edit a source `.md`, rebuild:
```bash
python3 revision-tools/build-pdfs.py                       # rebuild the default set
python3 revision-tools/build-pdfs.py <src-folder> <out-folder>   # one folder
```
