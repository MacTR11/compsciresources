...
- **GPU** — thousands of small cores; same operation on large data blocks **in parallel** (SIMD, data parallelism).
...
- **Won't parallelise well when:** data dependency (step needs prior result) / coordination overhead / inherently sequential algorithm. **Amdahl's Law:** serial fraction caps max speed-up.
