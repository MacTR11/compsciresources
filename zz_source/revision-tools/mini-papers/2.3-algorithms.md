...
A-B = 2,  A-D = 6,  B-D = 1,  B-C = 3,  B-E = 10,  C-E = 2,  D-E = 8
```
...
|------|---------|---|---|---|---|---|---------|
| Init | – | 0 | ∞ | ∞ | ∞ | ∞ | {} |
| 1 | **A** (0) | — | min(∞, 0+2)=**2** | ∞ | min(∞, 0+6)=6 | ∞ | {A} |
| 2 | **B** (2) | — | — | min(∞, 2+3)=**5** | min(6, 2+1)=**3** | min(∞, 2+10)=12 | {A,B} |
| 3 | **D** (3) | — | — | — | — | min(12, 3+8)=11 | {A,B,D} |
| 4 | **C** (5) | — | — | — | — | min(11, 5+2)=**7** | {A,B,D,C} |
| 5 | **E** (7) | — | — | — | — | — | {A,B,D,C,E} |

- Correct initialisation: A = 0, all others = ∞ (1).
- A settled first; B relaxed to 2, D relaxed to 6 (1).
- Visits nodes in correct distance order **A, B, D, C, E** (2).
- Correct relaxation of D to 3 via B, C to 5 via B, and E's tentative distance ∞ → 12 (via B) → 11 (via D) → 7 (via C) (2).
- Shortest distance **A → E = 7** (1).
- Shortest path **A → B → C → E** (2 + 3 + 2 = 7), found by tracing predecessors E ← C ← B ← A (1).
