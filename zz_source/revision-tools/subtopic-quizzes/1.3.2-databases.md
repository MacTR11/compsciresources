...
**15.** A car-hire firm uses a relational database with two tables:

`Customer(CustomerID, Name, Phone)`
`Rental(RentalID, CustomerID, CarReg, ReturnDate)`

(a) Identify the foreign key and the table it links to. **[1]**

(b) Write an SQL statement that joins the two tables to list each customer's `Name` together with the `CarReg` of every car they currently have on hire. **[3]**

(c) Explain how referential integrity prevents an invalid rental record. **[1]**
...
15. (a) **[1]** `CustomerID` in the `Rental` table is the foreign key; it links to `CustomerID` (primary key) in the `Customer` table.

(b) **[3]** Sample SQL:
```sql
SELECT Customer.Name, Rental.CarReg
FROM Customer
JOIN Rental ON Customer.CustomerID = Rental.CustomerID;
```
1 mark: `SELECT` of Name and CarReg; 1 mark: `FROM`/`JOIN` of both tables; 1 mark: correct `ON Customer.CustomerID = Rental.CustomerID`.

(c) **[1]** Referential integrity prevents a `Rental` record being created with a `CustomerID` that does not exist in `Customer`, so there can be no orphaned rental pointing to a non-existent customer.
